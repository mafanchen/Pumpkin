...
class VideoAdControl {
    ...
    fun showPauseAd(): Boolean {
        if (mPauseAd == null || TextUtils.isEmpty(mPauseAd?.adType)) return false
    ...
    private fun startPauseAdTimer() {
        ...
        mPauseAdTimer = Observable.interval(1, TimeUnit.SECONDS)
                ...
                .subscribe {
                    if (mPlayer != null) {
                        val time = mPauseAdLength - it
                        if (time <= 0) {
                            mPauseAdTimer?.dispose()
                            mPlayer?.getPauseAdTimeTextView()?.text = "关闭"
                            mPlayer?.getPauseAdTimeTextView()?.isEnabled = true
                        } else {
                            mPlayer?.getPauseAdTimeTextView()?.isEnabled = false
        ...
        mHeadAdTimer = Observable.interval(1, TimeUnit.SECONDS)
                ...
                .subscribe {
                    ...
                    if (mHeadAdTimeLog >= mHeadImageAdLength) {
                        mHeadAdTimeLog = 0
                        mPlayer?.setHeadAdTimeText("")
                        //超过5秒倒计时结束,停止计时
                        mHeadAdTimer?.dispose()
                        mPlayer?.startPlayVideo()
                    } else {
                        mPlayer?.setHeadAdTimeText("${(mHeadImageAdLength - mHeadAdTimeLog)}")
                    }
                }
    }
