...
/**
 * Created by enoch on 16/10/2017.
 */

public final class DeviceUtils {
    private DeviceUtils() {
        throw new UnsupportedOperationException("DeviceUtils can't initialize");
    }
...
//        return UTDevice.getUtdid(TestApp.getContext());

        String androidId = Settings.Secure.getString(TestApp.getInstance().getContentResolver(), Settings.Secure.ANDROID_ID);
        if (TextUtils.isEmpty(androidId) || AppConstant.ANDROID_BUG_ID.equals(androidId)) {
            return UTDevice.getUtdid(TestApp.getContext());
        } else {
            return androidId;
        }
    ...
    private static String getMacAddressByWifiInfo() {
        try {
            ...
            WifiManager wifi = (WifiManager) TestApp.getInstance().getSystemService(Context.WIFI_SERVICE);
            if (null != wifi) {
